var unify = (struct1, struct2, binding) => {
  ...
  while (workers.length) {
    ...
    if (Array.isArray(struct1) && Array.isArray(struct2)) {
      ...
      for (var i = 1, l = struct1.length; i < l; i++) workers.push(struct1[i], struct2[i]);
      ...
      if (contain(struct2, struct1)) {
        ...
        for (var i = 0, l = workers.length; i < l; i++) workers[i] = substitute(workers[i], eliminate);
...
var contain = (struct, variable) => {
  if (Array.isArray(struct)) for (var i = 1, l = struct.length; i < l; i++) if (contain(struct[i], variable)) return true;
...
var substitute = (struct, map) => {
  if (Array.isArray(struct)) {
    var result = [struct[0]];
    for (var i = 1, l = struct.length; i < l; i++) result.push(substitute(struct[i], map));
    return result;
  }
  return map(struct);
};
...
var merge = (binding, map, result) => {
  ...
  for (var i = 0, l = ks.length; i < l; i++) result[ks[i]] = substitute(binding[ks[i]], map);
  return result;
};
