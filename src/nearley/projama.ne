...
rule      -> _ tuple (__ "if" __ tuplelist):? "." _             {% d => ({ rule: { head: d[1], body: d.slice(2).flat(2).filter(x => !!x && typeof x !== 'string')[0]?.tuplelist ?? [] } }) %}
query     -> _ tuplelist "?" _                                  {% d => ({ query: d[1] }) %}
tuplelist -> tuple (_ "," _ tuple):*                            {% d => ({ tuplelist: d.flat(5).filter(x => !!x && typeof x !== 'string') }) %}
...
literal   -> ( string | number | boolean | bareword )           {% d => ({ literal: d[0][0] }) %}
